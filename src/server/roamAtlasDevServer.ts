...
import { buildEnvironmentPlanPrompt } from "../lib/prompts/buildEnvironmentPlanPrompt.js";
import { createLiveReloadServer } from "../platform/dev/liveReloadServer.js";
import { loadLocalEnv } from "../platform/env/loadLocalEnv.js";
import { createStaticAssetServer } from "../platform/http/staticAssetServer.js";
import {
  extractOpenAIText,
  parseJsonObject
...
import { createRoamAtlasApi } from "./createRoamAtlasApi.ts";
...
const liveReload = createLiveReloadServer({ repositoryRoot: root });
const { serveStaticAsset } = createStaticAssetServer({
  repositoryRoot: root,
  runtimeCacheRoot,
  runtimeCacheUrlPrefix: RUNTIME_CACHE_URL_PREFIX,
  liveReloadScript: liveReload.htmlScript
});
...
const countryImageService = createCountryImageService({
  repository: createCountryImageRepository({
    publicDirectory: path.join(root, "public", "country-cards"),
