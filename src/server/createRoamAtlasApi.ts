...
type RoamAtlasApiDependencies = {
  ...
  serveStaticAsset: (pathname: string) => Promise<Response>;
...
export function createRoamAtlasApi({
  ...
  app.get(liveReload.routePath, (context) =>
    liveReload.handleRequest(context.req.raw)
  );

  app.notFound((context) =>
    serveStaticAsset(new URL(context.req.url).pathname)
  );
