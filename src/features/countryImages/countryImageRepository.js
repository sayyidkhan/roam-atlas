...
export function createCountryImageRepository({
      ...
      for (const extension of SUPPORTED_IMAGE_EXTENSIONS) {
        ...
        try {
          ...
          if (fileStat.isFile()) {
            return {
              filePath,
              imageUrl: `${publicUrlPrefix}/${basename}${extension}`
            };
          }
        } catch {
  ...
  async function write(country, imageBytes, extension) {
    ...
    await writeFile(
      path.join(publicDirectory, `${country.slug}${extension}`),
      imageBytes
    );
    return `${publicUrlPrefix}/${country.slug}${extension}`;
  }

  function isLocalImageUrl(imageUrl) {
    return String(imageUrl ?? "").startsWith(publicUrlPrefix);
  }
