...
export function planNextFlipbookPage({
  currentNode,
  matchedNode,
  clickedPhrase,
  countryName = "selected country"
}: {
  clickedPhrase: string;
  countryName?: string;
  currentNode?: FlipbookNode;
  matchedNode: FlipbookNode | null;
}): FlipbookPlan {
  if (!matchedNode) {
...
function visualContextForNode(node: FlipbookNode): string {
...
function isFineGrainedDetourPhrase(
  phrase: unknown
): boolean {
