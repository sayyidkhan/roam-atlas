...
export function selectSemanticRegionForPoint(
  regions: SemanticRegion[],
  point: Point
): SemanticRegion | null {
  return (
    regions
      ...
      .sort((a, b) =>
        compareSemanticRegionHit(a, b, point)
      )[0] ?? null
  );
}
...
export function appendSemanticRegion(
  ...
  if (
    !phrase ||
    result.click?.resolver === "vlm_guard"
  ) {
  ...
  const existing = understanding.regions.find(
    (region) =>
      region.id === id ||
      (matchedNodeId &&
        region.matchedNodeId === matchedNodeId)
  );
  const nextRegion: SemanticRegion = {
    id,
    phrase,
    label: matchedNodeId
      ? pack.nodes[matchedNodeId]?.title ?? phrase
      : phrase,
    matchedNodeId,
    status: matchedNodeId
      ? "matched"
      : "unmapped_detour",
    bbox: boxAroundPoint(
      normalizedClick,
      matchedNodeId ? 0.22 : 0.12
    ),
    cacheClick: normalizedClick,
    confidence:
      result.click?.confidence ?? "general",
    confidenceScore: matchedNodeId ? 80 : 45,
    source:
      vlm.status === "resolved"
        ? "click_vlm"
        : "local_confirmed_click",
  ...
  if (existing) {
    existing.bbox = matchedNodeId
      ...
      : nextRegion.bbox;
    existing.phrase =
      existing.phrase || nextRegion.phrase;
    existing.cacheClick =
      nextRegion.cacheClick ?? existing.cacheClick;
    existing.updatedAt = nextRegion.updatedAt;
    existing.confidenceScore = Math.max(
      existing.confidenceScore ?? 0,
      nextRegion.confidenceScore ?? 0
    );
  } else {
...
export function createBaseUnderstanding(
  page: SemanticPage,
  {
    countrySlug,
    assetVersion
...
export function createUnresolvedClickResult({
  ...
  return {
    click: {
      ...
      nodeId: null,
      phrase:
        vlm.phrase ?? "unresolved illustrated detail",
      confidence: "unconfirmed",
      reason:
        "Runtime image click was not resolved with enough VLM confidence; refusing fallback hotspot navigation.",
      resolver: "vlm_guard"
    },
    page: {
      ...currentPage,
...
function compareSemanticRegionHit(
  a: SemanticRegion,
  b: SemanticRegion,
  point: Point
): number {
...
function semanticRegionClickDistance(
  region: SemanticRegion,
  point: Point
): number {
  const anchor =
    region.cacheClick ?? centerOfBox(region.bbox);
